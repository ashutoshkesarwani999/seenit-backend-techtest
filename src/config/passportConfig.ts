    ...
    new LocalStrategy(
      ...
      async (email: string, password: string, done:any) => {
        try {
          ...
          // if (!existingIUser[0].verified) {
          //   return done("User not verified.", null);
          // }

          const isPasswordCorrect = await bcrypt.compare(
            password,
            existingIUser[0].password
          );
          ...
          return done(null, existingIUser[0]);
        } catch (error) {
          done(error);
        }
      }
    ...
    new JWTStrategy(
      {
        ...
        secretOrKey: config.jwtSecretToken,
         //This secret token is needed so the JWTStrategy would be able to verify the encrypted password
